...
export default function App() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      await downloadSecurePDF(analysisResult, factPattern, watermark);
      logAudit({
        action: "EXPORT_PDF",
        userId: user.id,
        email: user.email,
              ...
              <div className="bg-white border-2 border-[#1E252B] relative p-6 sm:p-8">
                ...
                <div className="border-b-2 border-[#1E252B] pb-4 mb-6">
                  ...
                  <p className="text-xs text-[#4A5560] mt-1.5 leading-relaxed">
                    Provide the raw factual record, deed timelines, possession sequences, or statutory breaches. Upload PDF/text legal files or enter text manually. The secure local cascade engine will isolate critical parameters across 14 discrete analytical gateways.
                  </p>
                </div>

                {/* Local Document Ingestion Engine */}
                <div className="mb-6">
                  <DocumentUploader
                    currentTextLength={factPattern.length}
                    onTextExtracted={(extractedText, mode) => {
                      if (mode === "replace") {
                        setFactPattern(extractedText);
                      } else {
                        setFactPattern(prev => (prev ? prev + "\n\n" + extractedText : extractedText));
                      }
                    }}
                    onClearText={() => setFactPattern("")}
                  />
                </div>
                        ...
                        {factPattern && (
                          <>
                            <span className="text-[10px] font-mono text-[#C5A059] font-bold">
                              {factPattern.trim().split(/\s+/).filter(Boolean).length} words &bull; {factPattern.length} chars
                            </span>
                            <button
                              type="button"
                              onClick={() => setFactPattern("")}
                              className="text-[10px] font-mono font-bold text-red-600 hover:text-red-800 underline cursor-pointer"
                            >
                              Clear Text
                            </button>
                          </>
