...
// ============================================================================
// TEST INFRASTRUCTURE — Deterministic Fixture Factory
// ============================================================================
...
function makeRequest(overrides: {
  ...
  return {
    ...
    user: { userId: "test-runner", role: "DETERMINISTIC_TEST" },
...
function deterministicSnapshot(response: CaseAnalysisResponse): unknown {
  ...
  return canonicalStringify({
    ...
    stage0: {
      ...
      bainapatraRegistration: stage0?.bainapatraRegistration,
      treasuryDepositStatus: stage0?.treasuryDepositStatus,
      ancestorVitalStatus: stage0?.ancestorVitalStatus,
    ...
    stage2: {
      ...
      relevantSectionCount: stage2?.relevantSections?.length ?? 0,
      ...
      citationAudit: stage2?.citationValidationAudit,
    ...
    stage4: {
      plaintiffCount: stage4?.plaintiffs?.length ?? 0,
      defendantCount: stage4?.defendants?.length ?? 0,
    ...
    stage5: {
      ...
      timelineProgressCount: stage5?.timelineProgress?.length ?? 0,
    },
    outcome,
  });
}
...
// ============================================================================
// PHASE 1 — CORE DETERMINISM & FACT EXTRACTION
// ============================================================================
...
// ============================================================================
// PHASE 2 — DOMAIN & LEGISLATION
// ============================================================================

describe("PHASE 2: Domain Classification & Legislation", () => {
  describe("P2-01 Domain classification determinism", () => {
    it("Specific performance claim resolves to CONTRACT / TRANSFER_OF_PROPERTY domain", async () => {
      const r = await assertDeterministic(
        makeRequest({
          caseId: "P2-01",
          factPattern:
      ...
      expect(r.stage1.subsidiaryDomains.length).toBeGreaterThanOrEqual(0);
    });
  ...
  describe("P2-02 Legislation & precedent resolution", () => {
    it("Specific performance triggers Transfer of Property Act and Contract Act sections", async () => {
      const r = await assertDeterministic(
        makeRequest({
          caseId: "P2-02",
          factPattern:
      ...
      expect(r.stage2.relevantSectionCount).toBeGreaterThan(0);
    ...
    it("Precedent verification audit is deterministic and reports counts consistently", async () => {
      ...
      expect(r.stage2.citationAudit.totalCitations).toBeGreaterThanOrEqual(0);
      expect(r.stage2.citationAudit.validationStandard).toBe(
      ...
      expect(["PASS_100_PERCENT_DETERMINISTIC", "FAIL_UNVERIFIED_DETECTED"]).toContain(
        r.stage2.citationAudit.auditStatus
      );
    });
  });
...
// ============================================================================
// PHASE 3 — LIMITATION & LOCUS STANDI
// ============================================================================

describe("PHASE 3: Limitation & Locus Standi", () => {
  describe("P3-01 Limitation computation determinism", () => {
    it("Real refusal date extracted → valid timelineValidation with calculationType 'real_refusal'", async () => {
      const r = await assertDeterministic(
        makeRequest({
          caseId: "P3-01-REAL",
          factPattern:
            "Agreement dated 15 July 2020. Defendant refused to execute sale deed on 20 August 2020. Suit filed 2024.",
          submissionDate: "2024-03-01",
        })
      );
      expect(r.stage3.timelineValidation).toBeDefined();
      expect(r.stage3.timelineValidation?.calculationType).toBe("real_refusal");
      expect(r.stage3.timelineValidation?.isRefusalDateExtracted).toBe(true);
    });

    it("Missing refusal date → heuristic_6_months or missing_dates category", async () => {
      const r = await assertDeterministic(
        makeRequest({
          caseId: "P3-01-HEUR",
  ...
  describe("P3-02 Locus standi determinism", () => {
    it("Party extraction yields deterministic plaintiff/defendant arrays", async () => {
      ...
      expect(r.stage4.plaintiffCount).toBeGreaterThanOrEqual(1);
      expect(r.stage4.defendantCount).toBeGreaterThanOrEqual(1);
    });
...
// ============================================================================
// PHASE 4 — PLEADING & ISSUE FRAMING
// ============================================================================
...
// ============================================================================
// PHASE 6 — EQUITY, PROCEDURE & APPEAL
// ============================================================================
...
describe("PHASE 6: Equity, Procedure & Appeal", () => {
  describe("P6-01 Procedure determinism", () => {
    ...
    it("Timeline progress (CPC references) is deterministic", async () => {
      ...
      expect(r.stage5.timelineProgressCount).toBeGreaterThanOrEqual(0);
    });
  });
...
// ============================================================================
// PHASE 7 — FINAL GATE, OUTCOME & AUDIT
// ============================================================================
...
describe("PHASE 7: Final Gate, Outcome & Audit Integrity", () => {
  ...
  describe("P7-03 Audit hash determinism", () => {
    it("Output hash is identical across reruns", async () => {
    ...
    it("Different inputs produce different audit hashes", async () => {
...
// ============================================================================
// PHASE 8 — EDGE CASES & REGRESSION GUARDS
// ============================================================================
...
describe("PHASE 8: Edge Cases & Regression Guards", () => {
  describe("P8-01 Empty / minimal input", () => {
    it("Empty fact pattern does not crash and produces deterministic output", async () => {
      ...
      expect(r.stage0.atomicFactCount).toBe(0);
    });
  });
  ...
  describe("P8-03 Registration status fact meta (FIX #16 / FIX #9 regression)", () => {
    it("Bainapatra registration status is 'unspecified' when not mentioned, true/false when explicit", async () => {
