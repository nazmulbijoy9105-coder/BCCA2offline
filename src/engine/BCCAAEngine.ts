...
export class BCCAAEngine {
   ...
   * analysis.
   *
   * This method performs deterministic state synthesis only. It does not
   * invent substantive Bangladesh-law holdings. Unsupported or unresolved
   * upstream legal stages therefore force an indeterminate result and
   * human review.
   *
   * The upstream dependencies are optional only because the F0 critical
   * conflict path halts before those stages execute.
   */
  ...
  private buildResponse(
    ...
    sourceType: f.source.extractionMethod, extractionMethod: f.source.extractionMethod })),
      ...
      stage2: {
        ...
        citationValidationAudit: { totalCitations: 0, validatedCitations: 0, unverifiedCitations: 0, verifiedCount: 0, rejectedCount: 0, registrySignature: "BCCAA-REGISTRY-SIGNATURE", auditStatus: "NOT_EXECUTED", validationStandard: "Deterministic canonical-registry citation verification" },
  ...
  private computeOutputHash(response: CaseAnalysisResponse, _caseId: string): string {
    ...
    const semantic = {
      ...
      stage13: {
        overview: response.stage13?.overview ?? null,
        reliefDecree: response.stage13?.reliefDecree ?? null,
        costsApportionment: response.stage13?.costsApportionment ?? null,
        equitableBars: response.stage13?.equitableBars ?? null,
        executionPathway: response.stage13?.executionPathway ?? null,
        conclusion: response.stage13?.conclusion ?? null,
        confidence: response.stage13?.confidence ?? null,
        requiresHumanReview: response.stage13?.requiresHumanReview ?? null,
        humanReviewReason: response.stage13?.humanReviewReason ?? null,
